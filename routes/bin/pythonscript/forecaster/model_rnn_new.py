...
class LstmRNN(object):
    ...
    def save(self, step):
        ...
        self.saver.save(
            self.sess,
            os.path.join(base_path_nhuche+"modelsNewApproach3/new_model", model_name),
            global_step=step
        )

    def load(self, sess, data):
        ckpt = tf.train.get_checkpoint_state(base_path_nhuche+"modelsNewApproach3/new_model")
        if ckpt and ckpt.model_checkpoint_path:
            ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
            self.saver.restore(sess, os.path.join(base_path_nhuche+"modelsNewApproach3/new_model", ckpt_name))
